// fork of unmaintained https://raw.githubusercontent.com/mapbox/togeojson/master/togeojson.js
// eslint-disable consistent-function-scoping
// eslint-disable @typescript-eslint/no-explicit-async
...
export const toGeoJSON = (() => {
	...
	function attrf(x,y) {
		return Number.parseFloat(attr(x,y))
	}
	...
	function coordPair(x) {
		...
		let e
		if (ele) {
			e = Number.parseFloat(nodeVal(ele))
			if (!Number.isNaN(e)) {
				ll.push(e)
			}
		}
	...
	const t = {
		kml: (doc) => {
			...
			function kmlColor(_v) {
				...
				if (v.length === 8) {
					opacity = Number.parseInt(v.substr(0,2),16) / 255
			...
			function getTrack(node) {
				...
				for (let i = 0; i < segments.length; i++) {
					...
					if (line) {
						...
						if (heartRates.length || line.heartRates?.length) {
			...
			function getLineStyle(extensions) {
				...
				if (extensions) {
					...
					if (lineStyle) {
						...
						const opacity = Number.parseFloat(nodeVal(get1(lineStyle,"opacity")))
						const width = Number.parseFloat(nodeVal(get1(lineStyle,"width")))
