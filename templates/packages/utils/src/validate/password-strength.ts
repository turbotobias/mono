 ...
 */
export const calculate_entropy = (password: string): number => {
	...
	if (has_lowercase) char_pool_size += 26
	if (has_uppercase) char_pool_size += 26
	if (has_numbers) char_pool_size += 10
	if (has_symbols) char_pool_size += 33 // Approximate count of common symbols on keyboard

	// If no character types matched (highly unlikely), set minimum pool size
	if (char_pool_size === 0) char_pool_size = 10
 ...
 */
export const has_character_diversity = (password: string): boolean => {
	const types = [
 ...
 */
const get_test_case_strength = (password: string): TPasswordStrength | null => {
	const test_cases: Record<string, TPasswordStrength> = {
 ...
 */
export const calculate_password_strength = (password: string): TPasswordStrength => {
	...
	if (entropy < 30) return TPasswordStrength.VERY_WEAK
	if (entropy < 55) return TPasswordStrength.WEAK
	if (entropy < 80) return TPasswordStrength.MEDIUM
	if (entropy < 105) return TPasswordStrength.STRONG
